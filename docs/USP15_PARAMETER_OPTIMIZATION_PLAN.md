...
状态：**阶段 A 对齐审计通过但未通过 AF2 门控；阶段 B/C 已于
2026-07-25 完成；R2 未收敛且未启动扩量。**
...
六个条件均完成 50/50 个 backbone，Nextflow/direct trace 和 metrics
均无技术失败：

| 条件 | 生成数 | 通过全部 R2 backbone 过滤 | 进入 Phase C |
|---|---:|---:|---:|
| B1 | 50 | 0 | 0 |
| B2 | 50 | 3 | 3 |
| B3 | 50 | 3 | 3 |
...
S1/S2 通过率明显高于普通 Complex_beta 条件，但仍需以 AF2 复折叠为
最终门控，不能仅凭 backbone 指标判断成功。
...
- 20 个入选 backbone；
- 每个 backbone 在 temperature `0.05` 和 `0.10` 下分别运行；
- 40/40 个 LigandMPNN 条件技术成功；
- 120/120 条序列生成，binder Cys 数为 0；
- 120/120 条 `af2_model_1_multimer_tt_3rec` 预测完成；
- 技术失败数为 0；
- 同时通过三项 AF2 门控的设计为 0/120。
...
|---|---:|---:|
| iPAE | 22.01 | `<=10` |
| target-aligned binder RMSD | 23.20 Å | `<=2 Å` |
| binder pLDDT | 86.05 | `>=80` |

虽然个别设计的 binder pLDDT 超过 80，但 iPAE 和 target-aligned binder
RMSD 仍远离门控，不能将单项通过解释为合格候选。机器可读结果位于服务器
R2 目录的 `phase_c/reports/af2_metrics.csv` 和
`phase_c/reports/phase_c_summary.json`。最终状态为 `af2_gate_failed`，
下一步是报告 R2 未收敛，不启动自动扩量。
...
- [x] 六条件各完成 50/50 backbone 并记录逐项过滤数量。
- [x] Phase C 生成 120/120 条无 Cys 序列。
- [x] Phase C 完成 120/120 条指定 AF2 测试。
- [x] 记录 0/120 完整通过并停止，不放宽阈值或启动扩量。
