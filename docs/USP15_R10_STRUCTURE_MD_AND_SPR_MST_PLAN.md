...
当前执行状态为：10/10 个复合物已通过强制制备审计，真实 V100 CUDA
smoke 已通过 25,000 steps、5 个 ×10 ps 帧、3,336 个蛋白原子和生产期
无约束检查。原服务器任务现已在 `rank01/seed0` 的 NPT 平衡阶段暂停，
准备迁移至 Minerva；正式生产仍为 0/30。因此本节仍只记录冻结参数，
尚不构成 MD 稳定性结论；只有完成三个独立重复及分析后才填写结果表。
