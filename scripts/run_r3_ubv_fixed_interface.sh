...
phase_dir="${R3_FIXED_INTERFACE_DIR:-${campaign_dir}/r3/ubv_fixed_interface}"
...
mkdir -p \
    ...
    "${report_dir}" \
    "${phase_dir}/work/af2"
exec 9>"${phase_dir}/r3_fixed_interface.lock"
if ! flock -n 9; then
    echo "Another R3 fixed-interface driver holds the lock"
    exit 1
fi

"${python_bin}" "${campaign_dir}/scripts/prepare_r3_ubv_fixed_interface.py" \
    --input "${campaign_dir}/r3/ubv_positive_smoke/input/ubv15d_c11a.pdb" \
    --output "${input_dir}/ubv15d_fixed_interface.pdb" \
    --report "${report_dir}/preparation.json"

if [[ ! -f "${sequence_dir}/proteinmpnn.completed" ]]; then
    cp -f --remove-destination \
        "${input_dir}/ubv15d_fixed_interface.pdb" \
        "${sequence_work}/input/"
    (
        cd "${sequence_work}"
        "${python_bin}" \
...
if [[ "$(find "${sequence_pdb_dir}" -maxdepth 1 -type f -name '*.pdb' | wc -l)" -ne 3 ]]; then
    echo "Expected three fixed-interface ProteinMPNN outputs"
    exit 1
fi
...
if [[ ! -s "${jsonl}" ]] || [[ "$(wc -l < "${jsonl}")" -ne 3 ]]; then
    echo "Expected three fixed-interface AF2 records"
    exit 1
fi
...
touch "${phase_dir}/r3_ubv_fixed_interface.completed"
echo "R3 fixed-interface core redesign completed"
