...
phase_matrix="${R2_PHASE_A_MATRIX:-${campaign_dir}/config/r2_phase_a.tsv}"
phase_dir="${R2_PHASE_A_DIR:-${campaign_dir}/r2/phase_a}"
python_bin="${PYTHON_BIN:-${ovo_env_dir}/bin/python}"
af2_test="af2_model_1_multimer_tt_3rec"

if [[ ! -s "${phase_matrix}" ]]; then
    echo "Missing Phase A matrix: ${phase_matrix}"
    exit 1
fi

mkdir -p "${phase_dir}/runs" "${phase_dir}/reports" "${phase_dir}/work"
exec 9>"${phase_dir}/phase_a.lock"
if ! flock -n 9; then
    echo "Another R2 Phase A driver already holds ${phase_dir}/phase_a.lock"
    exit 1
fi
...
run_condition() {
    ...
    local source_pdb="${campaign_dir}/pilots/${source_pool}/metrics/output/${source_pool}/backbones_filtered/${backbone_file}"
    ...
    if [[ ! -f "${source_pdb}" ]]; then
        echo "Missing filtered source backbone: ${source_pdb}"
        return 1
    fi
    ...
    # Copy the selected backbone into the Phase A tree. OVO/Nextflow may mount only
    # this subtree into the container, so a symlink back to the R1 work tree can
    # become inaccessible inside LigandMPNN.
...
"${python_bin}" "${campaign_dir}/scripts/summarize_r2_phase_a.py" \
    --phase-dir "${phase_dir}" \
    --matrix "${phase_matrix}" \
    --csv-output "${phase_dir}/reports/af2_metrics.csv" \
    --json-output "${phase_dir}/reports/phase_a_summary.json"

touch "${phase_dir}/phase_a.completed"
echo "R2 Phase A completed; no scaling was started automatically"
